...
/** Reservable dates D+1..D+30 relative to KST today, as `YYYY-MM-DD` strings. */
export function datesInRange(now: number = Date.now()): string[] {
  return Array.from({ length: 30 }, (_, i) => kstToday(now + (i + 1) * DAY_MS));
}

/** Whether `date` (`YYYY-MM-DD`) falls within the D+1..D+30 reservable window. */
